...
#<%= "_remocao_dados.html" %>
 ...
 def nome_professor
    ...
    @teste = Trabalhado.find_by_sql("select t.professor_id from professors p left join trabalhados t on p.id=t.professor_id where t.ano_letivo = " + $data)
    if (@teste.nil?) or (@teste.empty?)
      ...
      @teste2 = Professor.find(:all, :conditions => 'id = ' + @teste)
