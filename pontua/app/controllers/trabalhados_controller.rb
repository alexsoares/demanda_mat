class TrabalhadosController < ApplicationController
  ...
  def index
    #@trabalhados = Trabalhado.find(:all,:conditions => ['professor_id = '+($teacher_id).to_s + ' and flag = 0'])
...
# =======================================
      ...
      $nomeprofessor = Professor.find_by_id($teacher_id).nome
              ...
              #@trabalho = Trabalhado.find(:all, :conditions => ['professor_id=' +($teacher_id).to_s, ' ano_letivo = ' + $data ])
      ...
      if $cont == 2 then
        ...
        if @inicial.empty? then
          render :update do |page|
            ...
            page.replace_html 'nomeprof', :text => 'Nome: ' + ($nomeprofessor)
          end
        else
...
#    @busca_trab = Trabalhado.find_by_sql("SELECT * FROM trabalhados where professor_id = " + $prof_id + " and flag = 0 ")
