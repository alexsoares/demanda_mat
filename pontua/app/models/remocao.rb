...
def before_create
      ...
      @atualiza_trabalhado = Trabalhado.find(self.professor_id, :conditions => 'ano_letivo = ' + $data.to_s)
      @ac_t = AcumTrab.find(self.professor_id)
    ...
    if self.remocao = 1 then
      ...
      if (@trab_atual).nil?
          ...
          @trab_atual.unidade_atual = t_at.unidade
      end
